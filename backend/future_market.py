...
#
# Five sections:
#   1. Career Outlook       — trend + government backing
#   2. Salary               — today / graduation / mid-career
#   3. Where the jobs are   — top cities from live data
#   4. Who will hire you    — top companies from live data
#   5. Competition level    — demand vs supply signal
#   6. Government push      — policy signal from news + LLM
#
# Sources:
#   SerpAPI        — current salary, cities, companies (already in system)
#   World Bank API — sector wage growth rate (free, no key)
#   Google News RSS — sector news headlines (free, no key)
#   NewsAPI        — additional headlines (free tier, optional)
#   Groq LLM       — classify news into outlook + competition + policy
#   Supabase       — weekly cache for demand, 30-day cache for salary
# ============================================
import requests
import feedparser
import json
...
def _generate_market_intelligence(career_title, sector, headlines, groq_key):
    """
    Single Groq call that generates all intelligence sections at once.
    Returns structured JSON covering outlook, competition, and policy.
    """
...
{{
  "outlook": {{
    ...
    "explanation": "One clear sentence a 12th grader would understand — why this career is growing or not in India"
  }},
  "competition": {{
  ...
  "policy": {{
    ...
    "scheme_name": "Name of government scheme or budget allocation, or null",
    "explanation": "One sentence explaining how this policy affects this career in India, or null"
  }},
...
Be specific to India. Use simple language. Base answers on headlines where possible.
Return ONLY the JSON object."""
    ...
    return {
        "outlook": {
            ...
            "explanation":       "Could not retrieve current market data for this career."
        },
        "competition": {
            "level":       "Moderate",
...
def get_future_market_data(career_title, sector, stream,
                           serpapi_salary_string, serpapi_companies,
                           serpapi_locations,
                           groq_key, news_api_key, supabase):
    """
    Master function — returns all 5 job market sections.
    ...
    Args:
        ...
        serpapi_companies     — list of companies from SerpAPI
        serpapi_locations     — list of cities from SerpAPI
        groq_key              — Groq API key
        news_api_key          — NewsAPI key (can be None)
        supabase              — Supabase client

    Returns dict with: salary, intelligence, companies, cities
    """
    # Salary projection
